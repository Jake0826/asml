...
from letter_recognition import recognize
...
while rval:
    ...
    for detection in results[:hand_count]:
        ...
        handRegion = frame[y-50:y+h+50, x-50:x+w+50]
        if not handRegion.size:
            continue
        # assume handRegion is a numpy array containing the image data
        retval, buffer = cv2.imencode('.jpg', handRegion)
        if not retval:
            # handle error
            pass
        # convert the buffer to bytes and pass it to your function
        image_bytes = buffer.tobytes()
        img = Image.open(io.BytesIO(image_bytes))

        recognize(img)
        
       #cv2.imwrite("export1.jpg", handRegion)
        ...
        color = (0, 255, 255)
        rectangle = cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
